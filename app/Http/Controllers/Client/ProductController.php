...
class ProductController extends Controller
{
    ...
    public function filterProduct(Request $request)
    {
        ...
        $arr = array_merge($session_request, $request->all());
        ...
        if(!empty($request->sortby)){
            ...
            }elseif($request->sortby =="date-desc"){
        ...
        if (!empty($request->years)) {
            $product = $product->whereIn('year', $request->years);
        }
    ...
    public function detail_product($cate,$type,$id)
    {
        ...
        ])->where('slug',$id)->first(['id','name','images','type_cate','type_two','category','sku','discount','price','content','size','description','slug','preserve','status_variant','created_at','species','variant','cate_slug','type_slug','qty', 'year']);
        ...
        $data['productlq'] = Product::where('status',1)->where('category',$data['product']->category)->where('id', '!=', $data['product']->id)->limit(12)->get(['id','category','name','status_variant','discount','price','images','slug','cate_slug','type_slug','description']);
    ...
    {
        $products = Product::where("name","LIKE",'%'.$request->keyword.'%')->where('status', 1)->select('id','category','name','discount','price','images','slug','cate_slug','type_slug','description','status_variant')->orderBy('id','DESC')
                  ->limit(8)->get();
        $view = view("layouts.product.search_live",compact('products'))->render();
        return response()->json([
            'html'=>$view
        ]);
    }
    ...
    {
        $keyword = $request->s;
         $data['list'] = Product::where('name', 'LIKE', '%'.$keyword.'%')->where('status',1)
         ->paginate(18);
    ...
    {
        $data = Product::where(['status' => 1, 'discountStatus' => 1])
            ->inRandomOrder()
            ->get()
            ->random();

        $view = view("layouts.product.getproajax",compact('data'))->render();
        return response()->json([
            'html'=> $view,
            'message' => 'success'
        ]);
